...
class ProcessData:
    ...
    def create_dataframe(self, kpi_name) -> pd.DataFrame:
        """Create a dataframe from the data"""
        kpi_constant = parse_to_constant_kpi(kpi_name)
        kpi = KPIs(self.data, axis=(1, 2), kpi=kpi_constant)
        return pd.DataFrame({kpi_constant.value: kpi.get_kpi(), "time": self.time})
